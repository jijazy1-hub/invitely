...
// GET /api/checkin?slug=event-slug — stats
...
// POST /api/checkin — scan and check in guest
export async function POST(req: Request) {
  try {
    const { slug, code } = await req.json();

    if (!slug || !code) {
      return NextResponse.json({ status: "invalid", message: "Missing slug or code." });
    }
    ...
    if (!event) {
      return NextResponse.json({ status: "invalid", message: "Event not found." });
    }

    // Find RSVP by unique code
    ...
    if (rsvp.status !== "CONFIRMED") {
      return NextResponse.json({
        status: "invalid",
        message: "Guest has not confirmed their RSVP.",
        guest: { name: rsvp.guest.name },
      });
    }
