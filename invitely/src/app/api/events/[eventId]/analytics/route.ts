...
import { getDevAnalytics, getDevEvent } from "@/lib/dev-store";
...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      console.error("[ANALYTICS] Database lookup failed, using local fallback:", error);
      event = await getDevEvent(user.id, params.eventId);
    }
    ...
    try {
      ...
      console.error("[ANALYTICS] Database aggregation failed, using local fallback:", error);
      const fallbackData = await getDevAnalytics(user.id, params.eventId);
      if (!fallbackData) return NextResponse.json({ error: "Event not found" }, { status: 404 });
